...
#        self.int3_tasks_per_sessions_dist = [x * self.int_impact[2] for x in self.baseline_tasks_per_sessions_dist]
        ...
        self.users_per_min = 0.0004  # calculated from data
    ...
    def simulate(self):
        ...
        # Get out of Simulator_Pipeline all records with t=ti
            ...
            for user_sim in current_pipe:
                ...
                if user< self.USERS_WANTED:     # skip last user
                    print_counter+=1
                    data = {'user_id': str(user), 'time': str(now + datetime.timedelta(seconds=time_stamp)),'project_id':'5733'
                            ,'workflow_id' : '10582' , 'subject_ids':'2', 'geo' :{ 'country_name': 'israel', 'city_name': 'yavne'
